  ...
  const fetchData = async () => {
    try {
      const [kpiRes, trendRes] = await Promise.all([
        axios.get('http://localhost:5000/api/reports/kpi', { headers: { Authorization: `Bearer ${token}` } }),
        axios.get('http://localhost:5000/api/reports/revenue-trend', { headers: { Authorization: `Bearer ${token}` } })
      ]);
      setKpi(kpiRes.data);

...
export function Profile() {
  ...
  const uploadAvatar = async (base64: string) => {
    try {
      await axios.patch(`/api/employees/${user?.id}/avatar`, { avatar: base64 }, {
  ...
  const fetchHistory = async () => {
    try {
      if (!user) return;
      const res = await axios.get(`/api/employees/${user.id}/attendance/history`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setHistory(res.data);
    } catch (e) {
  ...
  const markAttendance = async (action: 'MARK_IN' | 'MARK_OUT') => {
    try {
      await axios.post(`/api/attendance/mark`, { action }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchTodayAttendance();
      fetchHistory();

...
export function Schedule() {
  ...
  const fetchData = async () => {
    try {
      const aRes = await axios.get(`/api/appointments`, { headers: { Authorization: `Bearer ${token}` }});
  ...
  const cancelAppointment = async (id: string) => {
    try {
      await axios.patch(`/api/appointments/${id}/status`, { status: 'CANCELLED' }, { headers: { Authorization: `Bearer ${token}` }});
      fetchData();
    } catch (e) {
      console.error(e);
    }
